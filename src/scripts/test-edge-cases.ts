...
console.log('Case A (missing content):', caseA.success ? 'PASSED' : caseA.error.issues[0].message);


const caseB = createPostSchema.safeParse({ title: 'Valid Title', content: '' });
console.log('Case B (empty string content):', caseB.success ? 'PASSED' : caseB.error.issues[0].message);